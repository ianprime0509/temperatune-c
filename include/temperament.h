...
/**
 * Normalizes the note offsets within the temperament. This function
 * must be called after populating a temperament object; otherwise,
 * the pitches returned by temperament_get_pitch won't be in the correct
 * octave. temperament_parse will call this function by itself, so this is
 * only necessary if a temperament is constructed manually.
 *
 * Unexpected things will happen if you call this function with an invalid
 * temperament (e.g. one that doesn't have a note definition for the
 * octave base).
 */
void temperament_normalize(struct temperament *t);
...
/**
 * Sorts the note names in the given array in increasing order of pitch,
 * starting at the octave base.
 */
void notes_sort_names(const struct notes *notes, char *names[], size_t nnames);

/**
 * Stores copies of the names of all defined notes in the given array,
 * which must have enough space to fit them all.
 */
void notes_store_names(const struct notes *notes, char *names[]);
